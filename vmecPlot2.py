...
def main(file,name='',figures_folder='.', coils_curves=None, s_plot_ignore=0.2,savefig=True):
    ...
    gaussian_curvature = np.clip(np.array(surface.surface_curvatures()[:,:,1]).transpose(),-20,20)
    ...
    surf = ax.plot_surface(X, Y, Z, facecolors = cm.jet(norm(gaussian_curvature)), rstride=1, cstride=1, antialiased=False)
    ...
    norm = Normalize(vmin=gaussian_curvature.min(), vmax=gaussian_curvature.max())
