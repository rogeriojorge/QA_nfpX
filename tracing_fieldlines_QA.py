...
OUT_DIR = f"QA_nfp{nfp}/"

# Load in the boundary surface:
filename = (Path(__file__).parent / OUT_DIR / "input.final").resolve()
...
coils_filename = Path(__file__).parent / OUT_DIR / "coils" / "biot_savart_opt.json"
bs = simsopt.load(coils_filename)

surf.to_vtk(OUT_DIR + 'surface')
sc_fieldline = SurfaceClassifier(surf, h=0.02, p=2)
...
def trace_fieldlines(bfield, label):
    ...
    R0 = np.linspace(0.96, 1.25, nfieldlines)
